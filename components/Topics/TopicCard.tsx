...
export function TopicCard({ topic, viewMode }: TopicCardProps) {
  ...
  return (
    <>
      {/* Mobile Card Layout */}
      <div
        className="relative group cursor-pointer transition-all duration-300 md:hidden min-h-24 py-1 rounded-lg border-none bg-cream-200 flex items-center space-x-3 border-border hover:border-primary/30 shadow-sm"
        onClick={handleReadMore}
      >
        {/* Mobile - Avatar Image */}
        <div className="flex-shrink-0">
          <div className="w-20 h-20 rounded-br-3xl overflow-hidden bg-gradient-to-br from-primary to-accent border-2 border-primary/20">
            {topic.image ? (
              <Image
                src={topic.image}
                alt={topic.title}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            <button
              className={`bg-muted text-foreground rounded-full shadow-sm text-xs px-2 py-1 `}
            >
      ...
      <div
        ...
        onClick={handleCardClick}
      >
        <div className="relative flex flex-col items-start gap-0 p-0 bg-cream-200 backdrop-blur-sm rounded-lg border-none border-border shadow-sm hover:shadow-md hover:border-primary/50  transition-all duration-200 h-60">
          {/* Left: Square Image touching the left edge */}
          <div className="flex-shrink-0 flex justify-between items-center w-full h-1/2 rounded-l-lg overflow-hidden ">
            ...
            <div
              onClick={handleReadMore}
              className={`absolute right-1 font-thin bg-muted rounded-br-3xl rounded-tl-3xl backdrop-blur-md roundedxl flex items-center justify-center hover:text-primary text-foreground italic px-3 py-2 cursor-pointer transition-all duration-200 hover:scale-105 shadow h-10 w-[40%] mr-2 game-nav-buton shadow-primary/20`}
              // style={{
